...
RP2P is a tunneling component. It coordinates peers through an index, establishes a direct path, and transports TCP byte streams or UDP datagrams without interpreting or modifying the application's security model.

Peer authentication, user authentication, authorization, encryption, confidentiality, and application-level integrity belong to the application protocol or to optional external components composed around RP2P. This boundary is deliberate, not missing functionality or future work. SSH, HTTPS, and application-specific secure protocols can run through an RP2P tunnel without duplicating their security layer inside RP2P. Plaintext traffic is equally valid when the operator deliberately chooses it. RP2P does not impose a security model on transported applications.

`RP2P_PASS`, `RP2P_VIP`, and proof-of-work protect publisher registration and index capacity. They do not authenticate consumers or peers, and they do not encrypt or protect application payloads. Registration protection and tunnel security are intentionally separate concerns.

RP2P is designed for small, inexpensive, and inspectable deployments. The index coordinates peers without relaying application traffic, keeping its resource and operating requirements low. It is practical to run on modest VPS instances, home systems, community infrastructure, and other low-cost machines. The project prioritizes low operating cost, simple deployment, direct data paths, portability, inspectability, and composition.
...
The index control channel carries coordination and publisher-registration state only.
...
By default, the index is a lightweight public rendezvous server with open publisher registration. When `RP2P_PASS` is set, registration requires a password-derived proof.
...
`RP2P_PASS`, `RP2P_VIP`, and proof-of-work apply only to publisher registration and index capacity. Application protocols and optional external components define any consumer identity, access policy, or payload security selected by the operator.
